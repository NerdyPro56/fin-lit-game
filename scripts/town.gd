...
func _on_area_2d_5_body_entered(body: CharacterBody2D) -> void:
	if (body.name == "player") and !done1:
		player.set_position(Vector2(285, -122))
		body.cutscene = true
		done1 = true
		...
		textbox.queue_text("Yes? Well you seem nice enough to be a good wage sla- I mean worker! I need someone to cleanup for my workers!")
